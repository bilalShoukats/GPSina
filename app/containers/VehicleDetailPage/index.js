    ...
    useEffect(() => {
        ...
        if (vehicle) {
            ...
            if (formikRef.current) {
                ...
                formikRef.current.setFieldValue('make', vehicle.make);
                formikRef.current.setFieldValue(
                    'vehicleModel',
                    vehicle.vehicleModel,
                );
                formikRef.current.setFieldValue('engineNo', vehicle.engineNo);
                formikRef.current.setFieldValue('chassis', vehicle.chassis);
                formikRef.current.setFieldValue('mileage', vehicle.mileage);
                formikRef.current.setFieldValue('color', vehicle.color);
                formikRef.current.setFieldValue(
                    'vehicleType',
                    vehicle.vehicleType,
                );
                formikRef.current.setFieldValue('fuelType', vehicle.fuelType);
                formikRef.current.setFieldValue(
                    'fuelTankCapacityInLiters',
                ...
                formikRef.current.setFieldValue(
                    'loadingWeightCapacity',
                    vehicle.loadingWeightCapacity,
                );
                formikRef.current.setFieldValue(
                    'width',
                    vehicle.loadingAreaDimension.width,
                );
                formikRef.current.setFieldValue(
                    'height',
                    vehicle.loadingAreaDimension.height,
                );
                formikRef.current.setFieldValue(
                    'length',
                    vehicle.loadingAreaDimension.length,
                );
                formikRef.current.setFieldValue(
                    'interval',
                    vehicle.maintainanceInfo.interval,
                );
                formikRef.current.setFieldValue(
                    'next',
                    vehicle.maintainanceInfo.next,
                );
                formikRef.current.setFieldValue(
                    'reading',
                    vehicle.maintainanceInfo.reading,
                );
                formikRef.current.setFieldValue(
                    'company',
                    vehicle.insurance.companyName,
                );
                formikRef.current.setFieldValue(
                    'insuranceNumber',
                ...
                vehicle.insurance.expiredAt
                    ? formikRef.current.setFieldValue(
                          'expiredAt',
                          new Date(
                              vehicle.insurance.expiredAt * 1000,
                          ).toLocaleDateString('en-US'),
                      )
                    : formikRef.current.setFieldValue('expiredAt', null);
            }
        } else {
            props.history.goBack();
        }
