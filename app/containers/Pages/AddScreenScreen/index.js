...
class AddApiInfoScreen extends Component {
  ...
  schema = {
    ...
    apiDetail: Joi.string().regex(/^[a-zA-Z ]{20}/).required().error(errors => {
      errors.forEach(err => {
        switch (err.type) {
          case "string.regex.base":
              ...
              <Grid container spacing={3}>
                ...
                <Grid item sm={12} xs={12}>
                  <TextField
                    ...
                    name="apiDetail"
                    onChange={this.changeHandler}
                    value={this.state.apiDetail}
                    InputLabelProps={{
                      shrink: true,
                    }}
                    error={this.state.error.apiDetail && true}
                    helperText={this.state.error.apiDetail && this.state.error.apiDetail}
                    className="formInput"
                  />
                </Grid>
                <Grid item xs={12}>
