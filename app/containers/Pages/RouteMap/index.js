...
class EditRouteScreen extends Component {
  ...
  getRoute = () => {
    let body = {
      tripId: this.props.match.params.item,
      // companyEmail: this.props.user.companyEmail
    }
    console.log("response view body: ", body);
    this.props.apiManager.makeCall('fetchTripRoute', body, res => {
      console.log("response view body: ", res);
      if (res.code === 1019) {
        this.setState({ dataLoaded: true, mapRoutes: res.response });
      }
      else {
  ...
  render() {
    return (
      <Fragment>
        ...
        <h2 className="breadcumbTitle">Route from id : {this.props.match.params.item}</h2>
